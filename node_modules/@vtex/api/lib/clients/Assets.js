...
class Assets extends HttpClient_1.InfraClient {
    constructor(context, options) {
        super('apps@0.x', context, options, true);
        this.getAppFileByAccount = (app, path, nullIfNotFound) => {
            const locator = utils_1.parseAppId(app);
            const inflightKey = HttpClient_1.inflightURL;
            return this.http.get(this.route(this.context.account, locator, path), {
        ...
        this.getAppFileByVendor = (app, path, nullIfNotFound) => {
            const locator = utils_1.parseAppId(app);
            const vendor = locator.name.split('.')[0];
            const inflightKey = HttpClient_1.inflightURL;
            return this.http.get(this.route(vendor, locator, path), {
    ...
    getSettings(dependencies, appAtMajor, params = {}) {
    ...
    async getBuildJSONForApp(app, appVendorName, pick = []) {
        const pickArray = Array.isArray(pick) ? pick : [pick];
        const buildJson = await this.getFile(app.id, `dist/${appVendorName}/build.json`);
        const result = !ramda_1.isEmpty(pickArray) ? ramda_1.pick(pickArray, buildJson) : buildJson;
        result.declarer = app.id;
        return result;
    }
    async getSettingsFromFilesForApp(app, files = []) {
    ...
    async getFile(appId, file, nullIfNotFound) {
    ...
    fileRoute(workspace) {
        const appOrRegistry = ({ name, version, build }) => build
            ? `${workspace}/apps/${name}@${version}+${build}`
            : `master/registry/${name}/${version}`;
        return (scope, locator, path) => `/${scope}/${appOrRegistry(locator)}/files/${path}`;
    }
}
