...
class Apps extends HttpClient_1.InfraClient {
    constructor(context, options) {
        ...
        this.getAppsMetaInfos = async (filter, staleIfError) => {
            const { account, workspace, logger } = this.context;
            const metric = 'get-apps-meta';
            const inflightKey = HttpClient_1.inflightURL;
            try {
                const appsMetaInfos = await this.http.get(this.routes.Meta(), { params: { fields: workspaceFields }, metric, inflightKey }).then(ramda_1.prop('apps'));
                if (staleIfError && this.diskCache) {
                    appsStaleIfError_1.updateMetaInfoCache(this.diskCache, account, workspace, appsMetaInfos, logger);
                }
                if (filter) {
                    return ramda_1.filter(appMeta => !!ramda_1.path(['_resolvedDependencies', filter], appMeta), appsMetaInfos);
                }
                return appsMetaInfos;
            }
            catch (error) {
                if (staleIfError && workspace === 'master' && this.diskCache) {
                    return await this.diskCache.get(appsStaleIfError_1.getMetaInfoKey(account)) || [];
                }
                throw error;
            }
        };
